...
class ShipController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Handle type filter (now category)
        if ($request->has('category')) {
            $filteredCategories = $request->input('category');
            $query->whereIn('category_id', $filteredCategories);
            $activeFilters['categories'] = $filteredCategories;
        }
        ...
        $ships = $query->with('category')->get();
        
        // Get all available ship categories for filter options
        $shipCategories = \App\Models\Category::all();
        
        // Get all available amenities for filter options
        $allAmenities = Ship::whereNotNull('features')
            ->get()
            ->flatMap(function($ship) {
                return is_array($ship->features) ? $ship->features : json_decode($ship->features, true) ?? [];
            })
            ->unique()
            ->values()
            ->all();
        
        return view('user.ships.index', compact('ships', 'shipCategories', 'allAmenities', 'activeFilters'));
    }
    ...
    public function show($id)
{
    ...
    $similarShips = Ship::where('id', '!=', $id)
        ->where('category_id', $ship->category_id)
        ->limit(3)
        ->get();

    return view('user.ships.show', compact('ship', 'similarShips', 'nahkodas'));
}
    ...
    'nahkoda_id' => 'nullable|exists:nahkodas,id',
]);
