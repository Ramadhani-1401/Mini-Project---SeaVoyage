                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @php
                                    $defaultImages = [
                                        'Yacht' => 'https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?auto=format&fit=crop&w=100&q=80',
                                    ...
                                    $shipType = $ship->category ? $ship->category->name : '';
                                @endphp
                                @if($ship->image)
                                    <img src="{{ asset('storage/' . $ship->image) }}" alt="{{ $ship->name }}" width="100" class="img-thumbnail" style="height: 60px; object-fit: cover;">
                                @else
                                    <img src="{{ $defaultImages[$shipType] ?? 'https://placehold.co/100x60?text=Ship' }}" alt="{{ $ship->name }}" width="100" class="img-thumbnail" style="height: 60px; object-fit: cover;">
                                @endif
                            </td>
                            <td>{{ $ship->name }}</td>
                            <td>{{ $ship->category->name ?? 'N/A' }}</td>
