...
@else
<section class="hero-section" style="background-color: #1a252f; background-image: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80'); background-size: cover; background-position: center; height: 500px; position: relative; display: flex; align-items: center; justify-content: center;">
    <div class="hero-content container text-center" style="position: relative; z-index: 1;">
        <h1 class="display-3 fw-bold mb-4 text-white" style="text-shadow: 2px 2px 8px rgba(0,0,0,0.5);">Discover the Ultimate Yacht Experience</h1>
        <p class="lead mb-4 text-white" style="font-size: 1.3rem; text-shadow: 1px 1px 4px rgba(0,0,0,0.5);">Explore the waters in style with our premium yacht and ship rentals</p>
        <a href="{{ route('ships.index') }}" class="btn btn-light btn-lg" style="font-weight: 600; padding: 12px 40px;">Browse Fleet</a>
    </div>
</section>
@endif
            ...
            @forelse($featuredShips as $ship)
                <div class="col-md-6 col-lg-4 mb-4">
                    <div class="card ship-card h-100">
                        ...
                        @endphp
                        @php $shipType = $ship->category ? $ship->category->name : ''; @endphp
                        @if($ship->image)
                            <img src="{{ asset('storage/' . $ship->image) }}" class="card-img-top" alt="{{ $ship->name }}" style="height: 300px; object-fit: cover;">
                        @else
                            <img src="{{ $images[$shipType] ?? 'https://placehold.co/600x400?text=Ship' }}" class="card-img-top" alt="{{ $ship->name }}" style="height: 300px; object-fit: cover;">
                        @endif
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="ship-type">{{ $shipType }}</span>
