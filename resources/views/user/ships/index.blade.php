...
<div class="container py-5">
                            ...
                            @foreach($activeFilters as $key => $value)
                                ...
                                @elseif($key === 'categories')
                                    @foreach($value as $cat_id)
                                        @php $cat = \App\Models\Category::find($cat_id); @endphp
                                        <span class="badge bg-primary">Category: {{ $cat ? $cat->name : '' }} <i class="fas fa-times"></i></span>
                                    @endforeach
                        ...
                        @if(isset($shipCategories))
                            ...
                            <div class="form-check">
                                <input class="form-check-input filter-checkbox" type="checkbox" 
                                    ...
                                    name="category[]" 
                                    value="{{ $category->id }}"
                                    {{ isset($activeFilters['categories']) && in_array($category->id, $activeFilters['categories']) ? 'checked' : '' }}>
    ...
    <img src="{{ $ship->image ? asset('storage/'.$ship->image) : ($images[$shipType] ?? 'https://placehold.co/600x400?text=Ship') }}"
         class="card-img-top"
         alt="{{ $ship->name }}"
         style="height: 200px; object-fit: cover;">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">
                                <span class="badge bg-primary">{{ $shipType }}</span>
