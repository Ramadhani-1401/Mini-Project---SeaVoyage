        ...
        <div class="col-lg-8 mb-4">
                ...
                <div class="carousel-inner rounded">
                    ...
                    @else
                        ...
                        <div class="carousel-item active">
                            <img src="{{ $defaultImages[$shipType] ?? 'https://placehold.co/800x500?text=Ship' }}" class="d-block w-100" alt="{{ $ship->name }}" style="height: 500px; object-fit: cover;">
                        </div>
                    @endif
                </div>
            ...
            <div class="mt-4">
                ...
                <div class="d-flex align-items-center mb-3">
                    @php $shipType = $ship->category ? $ship->category->name : ''; @endphp
                    <span class="badge bg-primary me-2">{{ $shipType }}</span>
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info mb-3">
                        ...
                        <ul class="mb-0">
                            ...
                            <li>Maximum 2 bookings per ship category ({{ $ship->category ? $ship->category->name : '' }})</li>
                        </ul>
                    </div>
                    ...
                    <form action="{{ route('ships.book', $ship->id) }}" method="POST" id="bookingForm">
                        ...
                        <div class="mb-3">
                            <label for="nahkoda_id" class="form-label">Pilih Nahkoda</label>
                            <select class="form-select" name="nahkoda_id">
                                <option value="">-- Tanpa Nahkoda --</option>

                                @foreach($nahkodas as $nahkoda)
                                <option value="{{ $nahkoda->id }}">
                                {{ $nahkoda->nama }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span>{{ format_idr(500000) }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Service Fee</span>
                            <span>{{ format_idr(250000) }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2 fw-bold">
                            <span>Total</span>
                            <span>{{ format_idr((int) ($ship->price_per_day * 3 + 600000 + 150000)) }}</span>
                        </div>
            ...
            <div class="col-md-6 col-lg-4 mb-4">
                <div class="card ship-card h-100">
                    @if($similarShip->image)
                        <img src="{{ asset('storage/' . $similarShip->image) }}" class="card-img-top" alt="{{ $similarShip->name }}" style="height: 200px; object-fit: cover;">
                    @else
                        @php $similarType = $similarShip->category ? $similarShip->category->name : ''; @endphp
                        <img src="{{ $defaultImages[$similarType] ?? 'https://placehold.co/600x400?text=Ship' }}" class="card-img-top" alt="{{ $similarShip->name }}" style="height: 200px; object-fit: cover;">
                    @endif
                    <div class="card-body">
                        <h5 class="card-title">{{ $similarShip->name }}</h5>
                        <p class="card-text">
                            <span class="badge bg-primary">{{ $similarType ?? '' }}</span>
